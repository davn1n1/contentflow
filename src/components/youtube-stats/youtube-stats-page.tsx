...
import { DebugPanel, debugLog } from "@/components/debug/debug-panel";
import { DateRangePicker, getDefaultDateRange, type DateRange } from "./date-range-picker";
...
export function YouTubeStatsPage() {
          ...
          <div className="relative flex flex-col items-center gap-6">
            <div className="relative">
              ...
              <div className="relative w-20 h-20 flex items-center justify-center">
                <svg className="absolute inset-0 w-20 h-20 animate-[spin_2s_linear_infinite]" viewBox="0 0 80 80">
                  <circle cx="40" cy="40" r="36" fill="none" stroke="url(#ytGrad)" strokeWidth="2" strokeDasharray="180 100" strokeLinecap="round" />
                  <defs>
                ...
                <svg className="w-8 h-8 text-red-500 drop-shadow-[0_0_8px_rgba(255,45,85,0.5)]" viewBox="0 0 24 24" fill="currentColor">
            ...
            <div className="text-center space-y-2">
              ...
              <div className="flex items-center justify-center gap-1.5">
                <span className="text-xs text-muted-foreground">Obteniendo estadisticas del canal</span>
  ...
  return (
    <div className="space-y-6">
      <YouTubeHeader channel={data.channel} fetchedAt={data.fetchedAt} oauth={oauth} />
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        {/* Date range picker — visible for OAuth tabs when authenticated */}
      ...
      {/* Public tab content */}
      {activeTab === "overview" && (
        <OverviewTab videos={data.videos} subscriberCount={data.channel.subscriberCount} />
      )}
      {activeTab === "videos" && <VideosTab videos={data.videos} />}
      {activeTab === "engagement" && <EngagementTab videos={data.videos} />}
      ...
      {/* OAuth tabs — not authenticated */}
      {isOAuthTab && !oauth.isAuthenticated && (
        <div className="min-h-[40vh] flex items-center justify-center">
          <div className="glass-card rounded-xl border border-purple-500/20 p-8 max-w-md text-center">
            <div className="text-4xl mb-4">🔐</div>
            <h2 className="text-lg font-semibold text-foreground mb-2">Analytics privados</h2>
            <p className="text-sm text-muted-foreground mb-4">
              Conecta tu cuenta de YouTube para ver demographics, paises, ingresos y trafico.
            </p>
            <button
              onClick={oauth.login}
            ...
            {oauth.error && <p className="text-xs text-red-400 mt-2">{oauth.error}</p>}
          </div>
        </div>
      )}
      ...
      {activeTab === "demographics" && oauth.isAuthenticated && (
        <DemographicsTab data={analyticsData?.demographics ?? null} isLoading={analyticsLoading} error={analyticsError} />
      )}
      {activeTab === "geography" && oauth.isAuthenticated && (
        <GeographyTab data={analyticsData?.geography ?? null} isLoading={analyticsLoading} error={analyticsError} />
      )}
      ...
      {/* Debug panel — only renders on localhost?debug */}
      <DebugPanel />
    </div>
  );
}
