...
const statusConfig: Record<string, { icon: typeof CheckCircle; color: string; label: string }> = {
  converted: { icon: Rocket, color: "text-blue-400", label: "Listo" },
...
export default function RemotionProxyPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold">Remotion Proxy</h1>
        <p className="text-sm text-muted-foreground">
          Preview con assets proxiados via CDN — carga rapida para video, imagen y audio
        </p>
      </div>
      ...
      {!loading && timelines.length > 0 && (
        <div className="space-y-2">
          {timelines.map((entry) => {
            ...
            const videoClips = tl.tracks
              .filter((t) => t.type === "visual")
              .reduce((sum, t) => sum + t.clips.filter((c) => (c as { type: string }).type === "video").length, 0);
            ...
            return (
              <Link
                key={entry.id}
                href={`/remotion/${entry.id}/preview`}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 text-[11px] text-muted-foreground mt-0.5">
                    ...
                    {videoClips > 0 && <span className="text-blue-400">{videoClips} videos</span>}
                  </div>
                </div>
